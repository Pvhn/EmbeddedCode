...
void initHw()
{
    ...
    GPIO_PORTF_DIR_R &= ~(SW1_MASK | SW2_MASK);               // bit 4 is an input
    ...
    GPIO_PORTF_DEN_R |= SW1_MASK | SW2_MASK | GREEN_LED_MASK | RED_LED_MASK | BLUE_LED_MASK;
                                                         // enable LEDs and pushbuttons
    GPIO_PORTF_PUR_R |= SW1_MASK | SW2_MASK;                // enable internal pull-up for push button
}
